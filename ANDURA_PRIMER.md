...
**2026-05-31 → Recovery chat crăpat fără handover (scribe post-factum, SSOT re-sync):** Un chat anterior a crăpat mid-flight înainte de §F3.8 — treaba lui aterizase în git dar CHAT_STATE/PRIMER rămăseseră stale. Livrase: **harness numeric 15/15 PASS** (efemer — invariante BULK>MAINT>CUT, floor 1000F/1200M, AUTO==explicit, zero NaN, workout-uri reale lib-657, guard underweight, 250kg fără inversiune) + **smoke live Playwright 4 profile PASS** (Maria 65 / Marius 28 / aerobic-only F24 / AUTO M45, 0 erori) + `d5724ec3` **fix schedule** (DEFAULT_WEEK hardcoda 4 zile, ignora frecvența onboarding — Gigel: Maria 3x/săpt vedea 4) + `0d235cf9` **fix test nutrition fragil-la-dată** (targetObiectiv.month pe luna curentă înroșea CI la sfârșit de lună → mutat pe lună viitoare, deterministic) + `60bd1fe3` **legal GDPR** Privacy+T&C (Privacy.tsx/Terms.tsx + i18n en/ro + LegalPages.test.tsx). Primele două deja pe origin; legal pushed în recuperare împreună cu SSOT re-sync. Arc = execuție sub D077/D096, NU decizie nouă (DECISIONS netins). Cross-refs: `📥_inbox/HANDOVER_2026-05-31_crashed-chat-recovery.md` + CHAT_STATE §6.
