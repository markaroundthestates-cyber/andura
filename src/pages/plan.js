...
export function renderPlan() {
  ...
  // Date
  const pdEl = $('plan-date'); if(pdEl) pdEl.textContent = now.toLocaleDateString('ro-RO', {weekday:'long', day:'numeric', month:'long'});
  ...
  const pnEl = $('phase-name'); if(pnEl) { pnEl.style.color = pc.color; pnEl.textContent = phase; }
  const pdtEl = $('phase-detail'); if(pdtEl) pdtEl.textContent = pc.detail;
  ...
  const kcalDispEl = $('kcal-display'); if(kcalDispEl) kcalDispEl.textContent = kcal;
  const tdeeDispEl = $('tdee-display'); if(tdeeDispEl) tdeeDispEl.textContent = tdee;
  const lbmDispEl = $('lbm-display'); if(lbmDispEl) lbmDispEl.textContent = lbm;
  ...
  const clEl = $('checkpoints-list');
  if(clEl) clEl.innerHTML = checkpoints.map(cp => `
  ...
  const stEl = $('season-timeline');
  if(stEl) stEl.innerHTML = `<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--r);padding:14px 16px">
    ${timeline.map(t => {
