...
export const SYNC_KEYS = ['weights','kcals','prots','waters','wellbeing','logs','session-burns','session-ratings','muted','notif-enabled','suppl-list','early-stops','pr-records','phase-log','closed-days','step-streaks','steps-today','bf-override','phase-override','current-kcal','phase-change-date','readiness','unavailable-equipment','sf.userConfig', // sf.userConfig — bio + targetKg + equipment prefs (per audit night 2026-04-26 + ADR sync drift finding)
  ...
  'debug-recent',    // debug-recent — cross-device RECENT debug slice (2026-07-06, flag debug_recent_sync_v1). Object KEYED BY EVENT ID (id -> BehaviorEvent) so the object-merge (Object.assign local-wins) UNIONS ids across devices: a PC you never trained on shows the phone's last-3-session debug events. Written by debugLog.mirrorRecent (bounded to last 3 sessions); the full IDB behavior archive stays local-only, only this bounded slice syncs.
