...
/**
 * Reorder a split so each OWED cluster (carryover — scheduled last microcycle but
 * skipped) sitting in a LATE slot (especially the last) moves to the EARLIEST
 * position that does NOT increase same-cluster adjacency. PLACEMENT only — the
 * multiset of clusters is unchanged (same day-type counts), only the ORDER moves
 * so the skipped region is trained fresh. Lives here (next to the split-spacing
 * primitives it uses) so frequencyToSplit can apply it WITHOUT importing
 * carryoverBalance.js — that import would be a cycle (carryoverBalance imports
 * clusterForDay from here). detectOwedClusters (the impure log-reading signal)
 * stays in carryoverBalance.js and only imports FROM here (one-directional).
 *
 ...
 *
 * Reuses adjacencyCount + spaceOutSplit (above) to validate spacing. Legs
 * (lower/legs) are region-disjoint from push/pull/upper so they move freely; an
 * owed push/pull goes to the earliest non-adjacency-worsening slot. Empty owed →
 * the split is returned UNCHANGED (byte-identical). Pure, deterministic, stable.
 *
