...
amendments:
  ...
  - date: 2026-05-13e
    note: Bundle 4 workout-preview port + per-exercise refusal flow + mid-session augmentation LANDED `429c189` single atomic commit clean Bugatti single-concern — `feature/v2-vanilla-port` branch active vanilla port continuation Port-First-Then-React paradigm preserved invariant (NU React/JSX touched ZERO Step 2 React migration). Bundle 4 = vanilla DOM imperativ pattern via `$()` helper + native DOM API consistent ADR 005 original LOCK V1 stack discipline. workout-preview NEW src/ port first touch (per Bundle 3 path forward fresh chat strategic Daniel input mandatory consumed Bundle 4) + per-exercise inline button + handler onMissingEquipInlineClick(exerciseId) real signature integrate cu findAlternatives consumed orthogonal + toggleMissingEquipment registry + cascade alternative exhaustion mechanism + mid-session augmentation orchestrator behavior change (NU engine mutation ADR 026 §9 pure-function engines invariant preserved). Origin-rooted cascade tracking via `data-origin-exercise` DOM stable identity attribute NEW vanilla DOM imperativ pattern parity (anti-recurrence row identity mutates post-swap dar origin stable across cascade = counter cross-session accurate + exhaustion semantic preserved). Tests 3010 → 3111 PASS (+101 net new), 165 → 169 test files (+4 NEW + 1 augment). ZERO HARD CONSTRAINT §F3.12 violation (ZERO main branch + ZERO React/JSX + ZERO --no-verify + ZERO engine module mutation + ZERO `confirm()` prompt anti-paternalism preserved). §AR.21 4th consecutive validation effective post-codification ZERO slip surfaced (Bundle 1 + Bundle 3 + Bundle 3 follow-up + Bundle 4)
  - date: 2026-05-13d
