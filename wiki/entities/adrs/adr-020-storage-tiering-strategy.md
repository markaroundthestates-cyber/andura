...
amendments:
  ...
  - date: 2026-05-13e
    note: Bundle 4 workout-preview port + per-exercise refusal flow + mid-session augmentation LANDED `429c189` single atomic commit clean Bugatti single-concern — Tier 0 active rolling parity pattern S1.7 + S2 + Bundle 3 preserved invariant + 2 NEW localStorage keys Tier 0 parity pattern `wv2-missing-equipment` existing: (1) `wv2-skipped-exercises` permanent missing-equipment cascade additive per-exercise (auto-add când user tap "Nu am" preview row → registru global permanent NU 30d auto-expire rotation) + (2) `wv2-refusal-counter` cross-session ephemeral threshold tracking modal trigger per-exercise (counter cross-session origin-rooted via `data-origin-exercise` stable identity attribute pattern, modal "permanent?" la threshold 3 Co-CTO bias NU 5 paternalism NU 2 agresiv). Distinct semantic preserved invariant: `wv2-missing-equipment` = aparate fizice (permanent NU expire dacă user nu acquires equipment) + `wv2-skipped-exercises` = exerciții specifice skipped permanent missing-equipment cascade (semantic distinct vs aparate fizice) + `wv2-refusal-counter` = ephemeral cross-session threshold tracking modal trigger (counter cross-session pattern Daniel verbatim *"Daca refuza azi un exercitiu poate il face alta data. Daca nu are echipament sigur nu il face"*). User-driven edit oricând opt-in/opt-out preserved invariant Cont 2 grupuri vizibilitate (aparate-lipsa + aparate-excluse-din-antrenor) + reversibilitate Daniel verbatim *"in asa fel incat daca fac ceva nasol si bag un aparat sau exercitiu acolo din antrenor, sa fie reversibil procesul"*. Tier 0 = active rolling allows BOTH ephemeral time-bound AND permanent user-driven state AND cross-session ephemeral counter (3 use case patterns Tier 0 active rolling validated cumulative cross-bundle S1.7 + S2 + Bundle 3 + Bundle 4). Tests 3010 → 3111 PASS (+101 net new) vitest cluster cascade exhaustion mechanism + counter modal threshold + Cont 2 grupuri reversibilitate
  - date: 2026-05-13b
