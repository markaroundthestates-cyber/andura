...
D093 | 2026-05-28 | PROC | Autonomous Arc #5 — design storm 6 agenti Opus paraleli isolation:worktree pe mandat Daniel "fa-o Top Grade UX, sa atraga omul sa stea pe ea, sa nu fie painfull + in browser mai mica nu cat tot ecranul + fa animatiile + in background vreau animatii + preview themes e buguit". A1 (i18n Splash + Auth full 570 lines + Onboarding 8 steps + cross-cutting test locale pin afterEach +19 EN-leak assertions); A2 (i18n SubHeader Back + ExerciseMedia + RestOverlay Pauza/Sari pauza/recovering + ObiectivGoalCard 5 goal options + Ales badge + StatsGrid + ReadinessVerdict + engine title/warmup bridge + Workout chrome SETURI/EXERCITII + SessionTimer menu + ScheduleOverride; engine RO sentinel bridge pattern in React boundary fara engine churn); A3 (theme preview bug structural fix scoped `data-preview-palette=<id>` CSS var overrides per card + 4 paletes catalog global.css → true mini-snapshots, root cause: Clasic swatch foloseise tokens semantici `from-paper to-brick` care se rezolvau la paleta CURENTA, restul 3 hex literali); A4 (Progres 5-zone hierarchy reorg HERO+today→progress→body→nutrition cu staggered animate-card-rise + ZoneHeading helper + BackgroundAurora 3-blob palette-aware fixed inset-0 z-10 blur-80px opacity 0.08-0.12 drift 32/38/44s mounted in Layout + HeatMapWeekly soft brick gradient bars); A5 (Onboarding step transitions animate-fade-in-up + per-option stagger delay 150-525 + option-selected-ring palette-tint halo + step 8 confirm lucide icons Calendar/User/Target/Activity/Award/Scale/Ruler + Cont gradient avatar pebble + Lora tagline footer + Istoric+Calendar+VirtualSessionList hover-lift + PRNotificationBanner pop-in entrance + andura-pop-in keyframe scale 0.6→1.05→1 cubic-bezier overshoot); A6 (desktop phone-frame ≥768px CSS-only — Daniel "in browser sa fie mai mica nu cat tot ecranul" — radial-gradient stage tint --brick 4%+--ink 6% + 3-layer luxury shadow palette-aware accent halo color-mix(--brick) + delight-tier opt-in clase focus-ring-premium + card-hover-lift + num-display tabular-nums + sheen, mobile <768px ZERO change). Manager integration cherry-pick 21 commits onto main, 8 conflicts rezolvate manual (Onboarding.tsx 6 — keep A1 i18n t()/tArray() + A5 visual polish; i18nNoRoLeak.test.tsx 2 — keep A1 SPLASH+AUTH+ONB FINISH + A2 Wave F1 dedupe overlap tokens; routing.test.tsx auto-merge), combined commit `fffa09ff` Onboarding+cross-cutting (A1 commits 4+5 squashed due pre-commit hook test order dependency RO-pinned blocks leaking locale into EN-default downstream). Final **5330 verzi** + typecheck + lint clean + 290 test files. Pushed live origin/main `c2a59b34..d54655c1` 21 commits. WCAG AA preserved (Brain Coach 8.15:1 / Luxury 10.33:1 / Living Body 10.31:1 AAA). Reduced-motion-safe (all new animations gated via global * cap). Extinde D091+D092. | LOCKED V1 | DECISIONS.md §D093 + main origin

D092 | 2026-05-28 | ENG | Smoke fix Daniel `632fd0d4` — onboarding step 8 "Gata" button silently dies. Root cause: A2 #16 a adaugat `targetWeight` + `targetDate` ca OPTIONAL pe `OnboardingData` interface (user le seteaza in Progres > ObiectivCard post-onboarding, NU in wizard). `finalize()` itera `Object.keys(data)` si respinge la primul null → cele 2 optional default null faceau finalize sa cada immediat → completed ramanea false → click "Gata" arata toast "Completeaza toti pasii inainte de a continua" (gresit). Fix: enumerate explicit REQUIRED_FIELDS (Big 7: age/sex/goal/frequency/experience/weight/height), skip optional targetWeight/targetDate. Regression test pinned. Live deploy verde + push trigger Daniel. CI #649 + Deploy verzi pe `632fd0d4`. Plus "longevitate in onboarding" raport Daniel = ZERO in cod actual (verified grep src/react — doar comentarii istorice), eroare cached PWA pe telefonul lui (SW fix `52289184` din Wave F prinde update-ul automat la urmatoarea deschidere). | LOCKED V1 | DECISIONS.md §D092 + `632fd0d4`

D091 | 2026-05-28 | PROC | Autonomous Arc #4 — Wave E 4 agenti Opus paraleli (i18n DEEP FINISH true zero-RO-leak) + Wave F manager integration + SW update flow fix. Daniel critic: nu are sens smoke pe i18n incomplete → finish primul. Wave E: **E1 workout flow** (~100 keys: WorkoutPreview + PostRpe + PostSummary + ExitConfirmSheet + AaFrictionModal LOCK 9 + PainButton + SetLogInput + SetRatingButtons + AparatLipsaSheet; spec deviation gestionata: AlternativeModal/SkipModal nu exista, real = AaFrictionModal + Workout action row + EquipmentSwap deferred), **E2 body comp** (71 keys: BMRStrip + ProjectionStrip + NutritionInline + LogWeight + BodyData + WeightTimeline + WeightLogList; ObiectivCard already wired by C1), **E3 calendar+istoric** (~120 keys: Calendar7Day + CalendarHeatmap + Istoric + IstoricDetail + PrWall + PrWallRecent + RatingsStrip90Day + VirtualSessionList + months.full + weekdays.relativeShort + formatSessionsCount/formatSetsLabel helpers locale-aware pluralRo+_one/_other), **E4 settings+coach engine** (14 Settings + 8 Confirm + coach engine OUTPUT refactor — readiness.js + fatigue.js + coachVoice.ts + engineWrappers.ts emit semantic `key` field, React boundary localizes via t() + tArray helper NEW pentru list leaves FAQ/policy/terms/voice). **SW UPDATE fix** (Daniel smoke installed PWA stale): force `registration.update()` pe visibilitychange + setInterval 30min + initial nudge — installed PWA pe icoana primeste update auto fara reinstall. Note onestl: TWA/Play Store NU rezolva (Chrome wrapper, inner content tot SW cached). Wave F manager: merge --no-ff E2→E3→E1→E4 cu 2 conflicts rezolvate pe i18nNoRoLeak.test.tsx (bundles auto-merge clean), 'program' removed din forbidden tokens (cognate EN/RO). Final **5279 verzi** + typecheck + build clean (90 PWA precache, 1490 KiB; main chunk budget bump 160→175 KB). Security status verde pe Wave D anterior (Security Review workflow + npm audit 0 vulnerabilities + CI Validate + Deploy + Checkly synthetic). Extinde D090. | LOCKED V1 | DECISIONS.md §D091 + main local
...
D094 | 2026-05-29 | STRATEGY | Andura Pulse redesign LANDED + PUSHED LIVE — intregul app primar re-skinuit pe noul design system "Pulse" (mockup hand-built Daniel `04-architecture/mockups/interfata-noua/`) pe mandat "integreaza autonom cu subagenti + smoke + audit + security + push live cand termini tot + handover, andura nu are useri". Pulse = volt/aqua/ember/violet + Space Grotesk/Manrope/Space Mono self-hosted + motion --motion-aware reduced-motion-safe + desktop phone-bezel (translateZ(0) containing-block fix SubHeader-sticky/BottomNav-fixed) + aurora bg viu, paleta mapata pe token-urile EXISTENTE (--brick←volt) ca re-skin via token layer. **NEW DESIGN MASTER = interfata-noua/ — supersedes andura-clasic.html DESIGN MASTER designation din D015** (restul D015 — vanilla→React pivot + backend LOCK 1 — RAMANE valid; supersede DOAR clauza design-master NU intreg D015; ambiguous per D007 → flag Daniel confirmare, NU am flip-uit D015). **Luxury + Living Body palettes RETRASE** (Daniel "doar asta o sa ramana"), single Pulse light/dark. Arc: foundation `ce6fb5d4` + primitive partajate `9394aa62` (AuroraBackground/Ring/ReadinessOrb/Sparkline/PulseMark/Kicker/Pill + BottomNav reskin + aurora mount Layout) + 7 reskin primare worktree-isolated (Entry + Coach ReadinessOrb-hero + intensityMod onest + Progress Sparkline+Big-11-recovery-grid+kcal-hero-sus + History drill-down/virtualizare + Account inline-Dark/Light-toggle-wired-settingsStore + Workout-flow PostRpe-select-then-Save + Workout-LIVE ±dial+feel-card+PrFlash rest-overlay-non-modal-pastrat) + Wave 2c polish targetat (workout sub-flows + FatigueStrip + 3 RO banners→i18n). NU reskin brutal 37 secundare (mostenesc token-urile coerent, surgical-touch). Verificat: **5400 verzi** + typecheck + **fresh-eyes review GO** (never-delete invariants verificate la sursa: AaFriction LOCK9/refusal/wake-lock/setsDone/setSessionContext/finishSession-once/drill-down/consent-gate/medical-disclaimer-LOCK4 intacte; i18n paritate 1352 chei 0 diacritice; a11y pastrat; 0 suprafata securitate; 0 teste slabite) + smoke vizual live dark+light. **PUSHED LIVE** `5ffc80a8..4b200a39` 17 commits (branch-protection merge-commit+PR admin-bypassed), **CI #650 + Deploy #684 + QA #639 VERZI**, LIVE andura.app. Ramase non-blocant: deep cont/Settings*+confirm inherit-only, accent-swap picker deferred (Volt fix), EnergyCheck 2 dot-hex nit, ReactivateCard bold-{days} dropped, ~4GB worktrees cleanup. Extinde D093. | LOCKED V1 | DECISIONS.md §D094 + main 4b200a39 + handover 2026-05-29

D095 | 2026-05-29 | STRATEGY | Andura Pulse 1:1 parity GROUNDED fix + PUSHED LIVE — Daniel a deschis live post-D094 si a zis "ai facut-o sa arate ca varianta veche?". Avea dreptate la ce VEDEA: D094 mapase paleta pe token-urile vechi dar cardurile ramasesera FLAT/solide (`bg-paper2`, fara stratul glassy `--surface` din mockup) + service-worker servea build vechi din cache. Mockup `interfata-noua/` = legea (redesign intentionat login/workout/progress/themes-out). Goal LOCKED Daniel: paritate 1:1 totala 5 taburi. **Foundation glass** (`ef831591`): tokeni lipsa `--surface`/`-2`/`-solid`+`--bg-grad`+`--shadow-card`+`--glow` verbatim mockup + `.pulse-card`/`-tight`/`-glow` (glass fill+backdrop-blur+sheen+corner-wash); aurora era DEJA fidela (`pulse/AuroraBackground.tsx` — diagnoza initiala pe `BackgroundAurora.tsx` MORT corectata). **5 taburi→glass** prin 5 agenti Opus worktree file-disjoint cherry-pick: Istoric `744b0620` + Cont `7d39435c` + Progres `a8e7d650` + Antrenor `4638bb82` (fix bloc negru gol = card flat fara suprafata) + Workout `29786bbc`. **Themes SCOASE de tot** (`7d39435c`, Daniel "am renuntat expres la themes"): SettingsThemes+ruta+i18n+paleta multi-theme STERSE; accent picker Volt/Aqua/Ember/Violet + Dark/Light wired settingsStore.accent (swap `--brick` runtime validat, zero XSS). **Audit-fix** (`653cc191`): code-review MED 9 tile-uri Istoric/PrWall `pulse-card-tight` BARE (radius-only) → +`pulse-card` + fallback `@supports not(backdrop-filter)`→`--surface-solid`. **RO hardcodat reparat** (`147473e1`, Daniel a prins RO in app): workout sub-flows EnergyCause/CevaNuMerge/EquipmentSwap/AparateLipsa/AparatLipsaSheet i18n → en/ro.json 1422=1422 parity 0 diacritice + i18nNoRoLeak coverage (ScheduleOverride/PainButton erau deja curate). **Orb mereu-prezent** (`9b1a5899`, Daniel "cercul care respira de pe prima pagina nu l-ai vazut?" + LOCKED "mereu prezent"): ReadinessOrb exista+respira fidel dar gated (ascuns fara date — engine refuza verdict fara istoric, onest); hero acum MEREU randat — cu date scor real, fara date empty-state ONEST (orb dimmed + "—" + microcopy, ZERO scor fabricat, engine/getReadiness NEatins). **AaFriction glass** (`33f57c92`): popup greutate-peste-recomandat LOCK9 `bg-paper`→`pulse-card pulse-card-glow` ember, logica siguranta NEatinsa. **Login parity** (`8d5040ad`): "Welcome back"/"Bine ai revenit" gradient + buton Continua cu Google (refoloseste buildGoogleSignInUrl/handleGoogleSignIn B005, REST signInWithIdp), ierarhie mockup magic-link-primary + Google-ghost-secondary (supersede vechiul P-01 Google-primary 2026-05-11 — mockup nou SSOT). **Quality gate autonom** (Daniel "fa audituri/smokes/security ca eu n-am timp"): smoke vizual 5 taburi + workout flow + accent swap live; security review agent CLEAN (guard-uri rute intacte, accent fara injection, fara secrets); a11y agent no-blocker (text AA glass ambele teme); code-review 1 MED fixat; **verificare populata seed cont fals (localStorage browser, zero urma cod) → Progress recovery-grid+trend + History streak/sesiuni/calendar = 1:1 mockup, "fara chestii" era PUR cont gol NU design**. Baseline 5396 verzi/297 files + typecheck + build. **PUSHED LIVE** `4b200a39..8d5040ad` 14 commits (bypass admin regula PR, exit 0). **Daniel-side ramas**: buton Google apare DOAR dupa `VITE_GOOGLE_OAUTH_CLIENT_ID` + Google provider Firebase console (UI gata, degradare gratioasa ascuns); pixel-pass exhaustiv sub-ecrane profunde NEfacut (mostenesc sistemul + spot-checked); CI/deploy GH Actions de confirmat (gh nu-i pe PATH shell). Extinde D094. | LOCKED V1 | DECISIONS.md §D095 + main 8d5040ad + handover 2026-05-29_pulse-1to1-parity

D097 | 2026-05-31 | SAFETY | Tinta nutritie = goal+deadline-driven cat mai agresiv, singura limita de siguranta = floor kcal pe sex 1000F/1200M (rate-cap 1.5kg/sapt + 25% deficit / 15% surplus REMOVED) — Daniel CEO directive verbatim "legal e 1000 f si 1200 b minim... daca nu incercam sa ne adaptam la goalul userului nici drq nu foloseste aplicatia. Deci recomandam cat mai agresiv bazat pe goalul userului, dar niciodata sub 1000/1200". Problema reala: capurile intermediare de ritm (din `goalPhaseModel.sizeKcalForPhase`: 25% deficit / 1.5kg-sapt loss, 15% surplus / 0.5kg-sapt gain) plafonau agresivitatea PESTE minimul sigur → un user cu goal ambitios (ex F 160/78→60kg pana in Aug, sau M 181/108→90kg) primea 2173/2037 in loc sa coboare la floor, facand app-ul inutil pentru goalul lui. **Fix**: capurile de ritm SCOASE — deadline-ul (`requiredAbs = |target-current|×7700/zile`) conduce deficitul/surplusul DIRECT, cat de agresiv cere goalul. Singura limita ramasa = floor-ul pe sex (femei 1000 / barbati 1200) aplicat ULTIM in `finalize`, facut bulletproof (imposibil matematic sub floor pentru ORICE input — degradeaza gratios la banda sigura la floor/shift invalid NaN/<=0). Semnul fazei pastrat (cut=negativ, bulk=pozitiv). Post-fix: Case A (F 160/78→60/Aug) = 1000 floored, Case B (M 181/108→90/Aug) = 1200 floored. UI: nota onesta reframuita (`tdee-safety-limit-note` floored) — "tinta limitata la minimul sigur, la acest ritm obiectivul s-ar putea sa nu fie atins complet pana la termen" (ro+en). Plus BF validation feedback: valoare in afara 3-60% (ex 90) era acceptata-apoi-dropata silentios la save → acum mesaj inline vizibil + border rosu + aria-invalid (`profile-bf-range-error`). Plus freeze fix: `TDEEStrip` useEffect dep-array isi recalculeaza tinta LIVE la schimbarea goal-weight/deadline/greutate-logata (+ recompute pe focus/visibility pentru override-urile localStorage off-screen phase/bf). **Floor LOCK 8 (D-LEGACY-041) + sex-floor (D086) RAMAN VALIDE — floor-ul e PASTRAT, NU superseded; doar rate-cap-ul (sub-clauza D089 §A2 #16 `computeTargetKcalOverride cap asimetric -25%/+15% Aragon&Schoenfeld`) e scos** (nu supersede intreg omnibus D089 PROC peste o sub-clauza, per D007 literal-match + restul D089 nerelevant). Teste: rate-cap tests actualizate la noul contract floor-only (goalPhaseModel + nutritionSafety + targetKcal) + ADD hard floor never-below sweep + recompute freeze tests + BF range feedback tests. Suita verde **5757** + tsc + lint clean. Extinde D082+D086+D096. | LOCKED V1 | DECISIONS.md §D097 + src/react/lib/goalPhaseModel.ts + src/react/components/Progres/TDEEStrip.tsx + src/react/routes/screens/cont/settingsProfile/BodyCompSection.tsx + src/engine/bayesianNutrition/constants.js (floor)

D096 | 2026-05-30 | STRATEGY | Andura Progress-v2 redesign + body-model PHOTOREAL + overnight quality program (BUILD LANDED main, PUSH HELD ~91% real pending Daniel go) — Daniel mandat de seara "fa tot ce am discutat (smoke findings + Progress redesign + body-model) → reia harness+audituri stil dynamic-workflow + deep smoke 5-10 profile fictive + fix-loop pana flawless + confidence score real (NU umflat) 100% + push live + verify Actions/consola + vault/handover/igiena + doc la cafea; subagents autonom, access Chrome/API/GitHub". **Smoke findings Daniel reparate** (8 puncte): #1 bf-needs-both hint, #2 manual-bf clarificat (override fix, Bayesian deja calibreaza din intake+greutate), #3 "Ceva nu merge" Account hub explicat, #4 team copy "Daniel & Co" + Support solo-consistent, #5 body-model muscle map, #6 schedule-edit reactivity (Antrenor useEffect([]) → subscribe scheduleStore), #7 bodyweight exercise model (effectiveLoad=BW×fraction+added, PR/volume corecte, "X reps cu greutatea corpului"+added-weight; detectPR w<=0 bug fixat), #8 cold-start profile-scaled (110kg→25-31kg NU 10kg, BW×patternFraction×sex×experience floored). Plus date-NaN guard (sync poate aduce sesiuni fara ts), scroll-to-top pe tab-change (regresie din .app-scroll nav-fix). **Progress-v2 LOCKED** (mockup SUPERSEDED pt Progress — "punct de start nu lege"): tab reorg AZI/OBIECTIV/RECUPERARE/COMPOZITIE/ACTIUNI/TENDINTA, Target-Today single editable nutrition hero (kcal+protein, microcopy onest "sharpens as you log", fatigue+base-cal folded), Target Weight sus, MuscleBodyMap inlocuieste cercurile, Weight&BF trend jos; masuratori mutate in Profil, BodyData screen sters. **BF method LOCKED**: US Navy circumferinta (neck/waist/height +hip femei) DEFAULT + skinfold Jackson-Pollock 3-site OPTIONAL; chest/biceps/thigh circumferences DROPPED (masoara muschi nu grasime — a le baga in BF ar fi minciuna, vanity-tracking = noise). **Fatigue→kcal LOCKED**: minimal real deficit-ease only (fatigue mare + deficit activ → inmoaie deficitul spre mentenanta, cap +150, NICIODATA falsifica/scade TDEE). **Nutritie LOCKED**: 1 panou, edit = log intake real (feeds nutritionObservations→Bayesian TDEE), kcal+proteine only (NO carbs/grasimi, nu se introduce). **Body-model LOCKED**: randari foto-real Daniel (`assets-source/body/` jpg sursa → `public/body/*.webp` ~28KB shipped, originale PNG gitignored; `scripts/optimize-body-images.cjs` sharp pipeline refolosibil) + per-muscle recovery glow (volt/gold/ember mix-blend-screen, coords normalizate din ghidurile colorate), M/F dupa sex + Fata/Spate toggle, SVG-fallback onError, reduced-motion-safe. **Bodyweight LOCKED** + **cold-start scaling LOCKED** (vezi #7/#8). **Quality program**: re-audit confidence-scored (deep-smoke live 91/100 zero app console errors 4 profile ~60 rute toate randeaza zero orphan; static §03=88 §06=90 §07=90 §08=94 §09=~93 §11=82 — deduceri pur "static-only", suita+smoke acopera runtime); fix-loop a prins + reparat onboardingStore 8 RO leaks scanner-evading (.tsx-only) → harness extins la .ts stores TS_LEAK_KNOWN acum GOL + 4 alte .ts leaks (coachVoice/historyImportParser/scheduleAdapter/workoutStore) + NutritionInline orphan sters + Calendar7Day --accent→--brick + GDPR export sessions-store (Art.20) + a11y heading-order + desktop CTA-overlap (InstallPrompt+nav intercepta Confirm) + asset-bloat (deploy copia 16MB renderuri nefolosite → doar 4 webp acum). Baseline final **5557 verzi/308 files** + tsc clean + i18n 1538=1538 + golden-master neschimbat (cold-start scope no-data only). **PUSH HELD**: confidence ~91% real (NU literal 100 cum Daniel a cerut "100% real nu umflat") + zero FAIL-uri deschise dar PARTIAL-uri deferred (coach schedContext stub Phase-3, LWW multi-device same-day, AparateLipsa nav-hand-off, bodyweight eyes-on date-gated) + guardrail safety blocat push-to-live nesupravegheat la <100% cat Daniel doarme → CORECT held, asteapta Daniel go ("push") sau el trage trigger; main gata-de-push local nepushat (D031). Arc: ~30 agenti Opus worktree-isolated, manager cherry-pick serial, zero --no-verify. Extinde D095. | LOCKED V1 — PUSHED LIVE 2026-05-30 (Daniel morning trigger "push pe live am zis") `bcd15df7` + 6 live-smoke fixes `ade4a938` (cancel=zero-data + Progress polish + PULL title + pill/account) + CI Validate fix `951fcda3` (effectiveLoadKg unused→tsc exit255); deploy verificat live GREEN (build nou, cancel→readiness-empty, console clean). Actions Validate verificat local fresh (4 pasi curati); gh indisponibil + repo privat = NU citit direct. | DECISIONS.md §D096 + main d034bac6 + handover 2026-05-30
