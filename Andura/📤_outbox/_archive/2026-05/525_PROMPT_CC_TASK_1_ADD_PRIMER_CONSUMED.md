# PROMPT_CC TASK 1 — Create ANDURA_PRIMER.md în vault root

**Model:** Opus exclusively
**Scope:** TASK 1 of 3 — create singular briefing primer file at vault root
**Bugatti:** atomic single-concern commit, single file create
...
```bash
# Verify vault root doesn't already have ANDURA_PRIMER.md
test -f /c/Users/Daniel/Documents/salafull/ANDURA_PRIMER.md && echo "EXISTS — HALT" || echo "MISSING — proceed"
...
git status --short
# Expected: working tree clean (excluding .smart-env/)

git rev-parse --abbrev-ref HEAD
# Expected: feature/v2-vanilla-port

# Verify backup tag pre-execute pushed origin
git tag pre-add-andura-primer-2026-05-16
git push origin pre-add-andura-primer-2026-05-16
```
...
## §1 Create file `ANDURA_PRIMER.md` at vault root cu content EXACT verbatim

Write file `C:\Users\Daniel\Documents\salafull\ANDURA_PRIMER.md` cu content COMPLETE EXACT cum urmează (NU modify, NU adapt, NU compose — verbatim heredoc):
...
## §2 Verify file created cu content exact

```bash
wc -c /c/Users/Daniel/Documents/salafull/ANDURA_PRIMER.md
# Expected: ~10500-11500 chars

head -5 /c/Users/Daniel/Documents/salafull/ANDURA_PRIMER.md
...
grep -c "^## §" /c/Users/Daniel/Documents/salafull/ANDURA_PRIMER.md
# Expected: 8 (sections §1-§8)
```

---

## §3 Atomic single-concern commit Bugatti

```bash
cd /c/Users/Daniel/Documents/salafull

git add ANDURA_PRIMER.md
git commit -m "feat(vault): add ANDURA_PRIMER.md SSOT singular briefing fresh chat onboard

Daniel CEO directive 2026-05-16: \"vreau la next chat sa stii ce e andura,
ce face, cum functioneaza si ce trebuie sa fie. Sa stii unde am ramas,
ce e de facut si ce vreau eu\".

§1-§8 narrative compact ~10kB:
- §1 Ce este Andura (paradigm Bugatti + moat + personas)
- §2 Ce face (4-tab + 15 features + 8+1 engines pipeline §42.10)
- §3 Cum functioneaza (architecture pillars + metoda hibrida)
- §4 Ce trebuie sa fie (Pre-Beta FULL LOCK V2 + Daniel Gates 100%)
- §5 Unde am ramas (Triple LANDED + Big 11 + reglaj wave 4)
- §6 Ce e de facut (backlog ordered §AR.30/31 + P4 + Bugatti audit)
- §7 Ce vrea Daniel (Co-CTO autonomy + daniel-isms + format)
- §8 Cross-refs authority

Maintained per handover. Bugatti craft.
...
## §4 Tests baseline verify preserved

```bash
npm run test:run --silent 2>&1 | grep "Tests"
# Expected: Tests 3734 passed (3734) — ZERO regression (doc-only)
```

---

## §5 Status output în `📤_outbox/LATEST.md` § structured TASK 1 raport

Update `📤_outbox/LATEST.md` ca raport intermediar TASK 1 cu format:

```
# LATEST — TASK 1 of 3 ANDURA_PRIMER.md LANDED 2026-05-16

## Task
TASK 1 of 3 — Create ANDURA_PRIMER.md SSOT singular briefing vault root

## Status: COMPLETE

## Pre-flight
- Backup tag pre-add-andura-primer-2026-05-16 pushed origin ✓
- Working tree clean ✓
- Tests 3734 PASS pre-execute ✓

## Modificare
- NEW file: ANDURA_PRIMER.md (~10.5kB §1-§8 narrative)

## Build+Tests
Tests 3734 passed (3734) — preserved EXACT ✓
ZERO src/ touched (doc-only operation)

## Commits
- <SHA> | feat(vault): add ANDURA_PRIMER.md SSOT singular briefing fresh chat onboard
...
## Next action
TASK 2 of 3 → archive wiki/ → 99-archive/wiki-pre-2026-05-15/ + create dir
```
...
- ✅ ZERO src/ touched (doc-only)
- ✅ Single-concern atomic commit Bugatti
- ✅ Backup tag pushed origin pre-execute
- ✅ Tests 3734 PASS preserved EXACT
- ✅ Pre-commit hook ran (no --no-verify bypass)

---

🦫 **TASK 1 of 3. Atomic single-concern. Bugatti craft. Standalone deliverable — if TASK 2/3 fail, TASK 1 LANDED preserved.**
